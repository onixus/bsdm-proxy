...
- **eBPF/XDP lab run on a real kernel** — `ci/ebpf-lab/` (Jenkins job `bsdm-ebpf-lab`) builds the proxy on a Linux agent and runs `scripts/run-ebpf-lab-smoke.sh` in a `--privileged --network none` container, with XDP on a veth pair so the agent's NICs are untouched. The smoke gained a datapath check (`EBPF_PROBE_NETNS`): pings from a blocked source must be dropped by the kernel and show up in the API and Prometheus counters.
...
- **eBPF/XDP filter did not work on a real kernel** — found by the first run on Linux (7.2, Arch host, `ci/ebpf-lab/`). The XDP object failed to build on Debian/Ubuntu (`asm/types.h` not found: the multiarch include directory is now passed) and, once built, would not load (`BTF is required`: it is now compiled with `-g`). With the program attached, every block and unblock failed: the maps were looked up by name, but the kernel truncates names to 15 characters and both blocklists became `bsdm_blocked_ip`; they are now resolved by id through the attached program. The key bytes were also passed to `bpftool` as one argument (`error parsing byte`). Blocking IPv4 and IPv6 sources now really drops their packets, and the kernel counters reach `/api/ebpf/stats` and `bsdm_proxy_ebpf_*_dropped_total`. A leftover `bpf/xdp_drop.o` built without `-g` must be deleted.
...
- **eBPF/XDP lab path** — `scripts/run-ebpf-lab-smoke.sh` (preconditions, BPF object
  build, full `/api/ebpf/*` lifecycle, `bsdm_proxy_ebpf_*` metric assertions, detach
  teardown; skips with exit 0 on non-Linux hosts) and
  [`docs/features/ebpf-xdp.md`](docs/features/ebpf-xdp.md) — lab-only warning,
  kernel/capability requirements, arming model, full Control API reference, metrics,
  unload procedure and known limitations. Linked from `docs/project-status.md`,
  `docs/roadmap.md`, `docs/README.md` and the Day-1 checklist in
...
- **eBPF/XDP arming gate** (`proxy::ebpf`, `proxy::control_api`) — the kernel XDP
  filter can no longer be switched on by the control plane alone. `EbpfXdpConfig`
  gained a read-only `runtimeEnableAllowed` flag, sourced exclusively from the
  process environment (`EBPF_XDP_ENABLED=true` attaches at boot,
  `EBPF_XDP_ALLOW_RUNTIME_ENABLE=true` only arms), mirroring the TI enforcement
  fail-safe (`TI_ENFORCEMENT_MODE=enforce`). While un-armed,
  `PUT /api/ebpf/config {"enabled": true}` returns `403 Forbidden` and applies
  nothing, a request body claiming `runtimeEnableAllowed: true` is ignored
  (`skip_deserializing`), and `EbpfXdpManager::new` downgrades an enabled-but-un-armed
  configuration to disabled instead of loading a program. Disabling stays allowed
  in every state. New gauge `bsdm_proxy_ebpf_armed` (pilot invariant: `0`), new
  field `runtimeEnableAllowed` in `GET /api/ebpf/stats`, and explicit kill-switches
  in `deploy/compose/docker-compose.pilot.yml` and `bsdm-proxy.env`.
