...
## Аллокации и contention на hot path

Помимо переключателей выше, сам request path устроен так, чтобы на запрос
приходилось как можно меньше heap-аллокаций и как можно меньше времени под
блокировками. Это работает всегда и не требует настройки.

| Этап | Что делается вместо «наивного» варианта |
|------|------------------------------------------|
| ACL | Правила матчатся по ссылке на `arc-swap` snapshot — без клонирования всего набора правил на каждый запрос; regex резолвится по индексу правила, а не по хешу строки-паттерна |
| Policy decision cache | Шардированный `RwLock` вместо одного глобального `Mutex`; ключ рендерится в thread-local буфер, поэтому lookup не аллоцирует; вытеснение — по ограниченной выборке, а не полным сканом под блокировкой |
| Домен из URL | Быстрый срез host для обычных ASCII-URL; полный `Url::parse` только для IDNA/punycode, percent-escape и IPv6-литералов |
| Категоризация | Цепочка суффиксов домена — заимствованные срезы host, без `Vec<String>` из склеенных суффиксов |
| Cache key | SHA-256 остаётся (это wire format для ICP/HTCP/cache digest), но hex пишется в стековый буфер — одна аллокация вместо двух |
| L1 shard | Быстрый mixer вместо SipHash по 64-байтному hex-ключу (ключ и так высокоэнтропийный, клиент им не управляет) |
| События | `CacheEvent` не собирается вовсе, если ни Kafka, ни HTTP-sink не настроены |
| ICAP | Map заголовков запроса материализуется только когда REQMOD/RESPMOD включён |
| Иерархия | Заголовки запроса клонируются под peer fetch только если hierarchy сконфигурирована |
| Rate limit | При выключенном лимите (default) заголовки не сканируются вообще; API-ключ ищется хеш-лукапом, а не перебором заголовков |
| Метрики | Лейблы method / status / cache status берутся из статического словаря, без `String` на запрос |
| Sampling | `KAFKA_SAMPLE_RATE` / `METRICS_SAMPLE_RATE` считаются атомарным счётчиком, а не броском RNG — доля ровно 1 из N |

> Профилировать эффект удобно через `./scripts/perf-profile.sh` и
> `./scripts/run-proxy-benchmark.sh` — см. разделы выше.
