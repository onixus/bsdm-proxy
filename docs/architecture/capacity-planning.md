...
| dns-sinkhole (DoH/DoT)* | 2 | 2 | 2 GB | 10 GB |
| Redis L2 | 2 | 4 | 32 GB | 100 GB |
| Kafka | 3 | 4 | 16 GB | 200 GB |
| ClickHouse | 1 или 3 | 8 | 32 GB | 500 GB NVMe |
| ml-worker / indexer* | 2 | 4 | 8 GB | 20 GB |
| Vector DB (AI Cache)* | 1 | 4 | 16-32 GB | 100 GB SSD |

| Итого (Базовый стек) | vCPU | RAM | Диск |
|----------------------|------|-----|------|
| **CH ×1** (типичный старт) | **60** | **208 GB** | **~1.5 TB** |
| **CH ×3** (HA analytics) | **76** | **272 GB** | **~2.5 TB** |

*\* Компоненты со звездочкой опциональны и активируются по мере необходимости (DNS-фильтрация, ML Threat Scores, AI LLM Cache).*

Monitoring (Prometheus / Grafana / Alertmanager) — обычно на отдельных нодах; в итоги выше не входит
(~2–4 vCPU / 4–8 GB).

Data plane при ~350 peak RPS сам по себе легче, чем 4×8 vCPU: запас заложен под MITM, ICAP/ICAPS инспекцию, Wasm хуки,
иерархию и рост. Тяжесть рефа — **Kafka + ClickHouse + Redis L2 + Vector DB**.

### Влияние новых модулей на производительность (Add-ons)

Архитектура BSDM-Proxy поддерживает ресурсоемкие модули. Учитывайте их при сайзинге:

| Модуль | Влияние на ресурсы | Примечание |
|--------|--------------------|------------|
| **eBPF / XDP** | **Снижает** CPU proxy | Аппаратный дроп пакетов (ACL/Threats) на уровне драйвера NIC (0% userspace CPU). Высвобождает ресурсы под полезный трафик. |
| **Wasm Plugins** | **Повышает** CPU proxy | Изолированное выполнение хуков. Увеличивает latency на 0.5–2ms/req. Обязательны Fuel limits. |
| **DoH / DoT (Encrypted DNS)** | **Повышает** CPU sinkhole | Дополнительные расходы на TLS-рукопожатия для DNS-запросов (TCP/853, HTTPS/443). |
| **ICAP TLS (ICAPS)** | **Повышает** CPU proxy | Добавляет криптографический оверхед при связи с внешними DLP/AV серверами по защищенному каналу. |
| **AI Semantic Cache** | **Требует Vector DB** (RAM) | Кэширование LLM. Требует внешней Vector DB (Qdrant/Milvus), потребляющей от 16 GB RAM на хранение эмбеддингов. |
| **Cluster Mesh (gRPC)** | Незначительное | Более эффективная и легковесная синхронизация кэша по сравнению с broadcast. |
