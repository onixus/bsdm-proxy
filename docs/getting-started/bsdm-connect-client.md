...
> **Только лаборатория (Beta).** Локальный агент, `bsdm-connect` и путь
> AmneziaWG **не входят в production-контур пилота и не поддерживаются в
> продакшене**. Зрелость по [project-status.md](../project-status.md): **Beta
> (lab)**; в матрице Day-1 пилота они отмечены **OFF**
> ([pilot-deployment.md](pilot-deployment.md)). Использовать только на
> лабораторных устройствах: пакеты не подписаны/не нотаризованы, гарантий
> доступности и поддержки для этого пути нет (issue #331).
