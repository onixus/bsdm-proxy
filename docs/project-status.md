...
Текущая версия Cargo workspace: **`0.9.3`**. Версию нужно сверять с
...
| Agent (Phase C) | Local policy agent spike | Beta (lab) | Enroll + CSR certs + events; optional **control mTLS port** (`CONTROL_MTLS_*`). Multi-OS / policy push / CRL reserved. [pilot-agent.md](getting-started/pilot-agent.md). |
