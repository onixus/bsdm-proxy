...
| Agent (Phase C) | Local policy agent spike | Beta (lab) | Enroll, CSR, events, push, mTLS, CRL, lab OCSP JSON + **RFC 6960 DER OCSP**, gRPC policy watch/push (`--features grpc`), Admin `/devices`, `agent_api`. Multi-OS reserved. [pilot-agent.md](getting-started/pilot-agent.md). |
