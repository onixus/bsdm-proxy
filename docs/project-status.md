...
Текущая версия Cargo workspace: **`0.9.1`**. Версию нужно сверять с
...
| Agent (Phase C) | Local policy agent spike | Beta (lab) | `examples/agent-spike`: policy pull + local SNI/pinning/mode + heartbeat; control `GET/POST /api/v1/agent/*`, devices registry. Не multi-OS production. [pilot-agent.md](getting-started/pilot-agent.md). |
