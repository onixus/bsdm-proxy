...
export function DistributionAnalytics({ labels, statusSegments, cacheSegments, decisionSegments, topDomains, topClients, topBlocked }: DistributionAnalyticsProps) {
  return (
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Panel title={labels.topDomains}><BarList items={topDomains.map(([label, value]) => ({ label, value }))} /></Panel>
        <Panel title={labels.topClients}><BarList items={topClients.map(([label, value]) => ({ label, value }))} /></Panel>
        <Panel title={labels.topBlockedDomains}>
          {topBlocked.length === 0 ? <EmptyState message={labels.noBlockedRequests} /> : <BarList items={topBlocked.map(([label, value]) => ({ label, value }))} />}
        </Panel>
      </div>
    </>
  )
