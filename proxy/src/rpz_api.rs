...
impl RpzApiState {
    ...
    fn compile_zone(&self, state: &RpzStateFile) -> Result<(), String> {
        let mut out = String::from("; Generated by BSDM RPZ API — do not edit by hand\n$TTL 300\n\n");
        let mut lists: Vec<&RpzList> = state.lists.iter().filter(|l| l.active).collect();
        lists.sort_by_key(|l| std::cmp::Reverse(l.priority));
        for list in lists {
            out.push_str(&format!("; list {} ({})\n", list.id, list.name));
            let content_path = self.state_dir.join("lists").join(format!("{}.txt", list.id));
            if let Ok(content) = std::fs::read_to_string(&content_path) {
                out.push_str(&normalize_list_content(&content, &list.format, &list.default_action));
            }
            out.push('\n');
        }
    ...
    async fn test_domain(&self, query: &str) -> Response<Body> {
        ...
        lists.sort_by_key(|l| std::cmp::Reverse(l.priority));
        for list in lists {
            let path = self.state_dir.join("lists").join(format!("{}.txt", list.id));
    ...
    async fn stats(&self) -> Response<Body> {
        ...
        let total_rules: u64 = active.iter().map(|l| l.rule_count).sum::<u64>()
            + g.custom_rules.len() as u64;
        json_ok(&serde_json::json!({
    ...
    async fn add_rule(&self, body: Bytes) -> Response<Body> {
        ...
        let rule = RpzRule {
            ...
            domain: input.domain.trim().trim_end_matches('.').to_ascii_lowercase(),
            action: input.action,
            target_ip: None,
            target_cname: None,
    ...
    for line in content.lines() {
        ...
        match format {
            ...
            RpzListFormat::DomainList => {
                let dom = t.split_whitespace().next().unwrap_or("").trim_end_matches('.');
                if !dom.is_empty() {
                    out.push_str(&domain_policy_line(dom, action));
                }
            }
...
fn rule_to_zone_line(rule: &RpzRule) -> String {
    if rule.action == RpzAction::Passthru {
        return format!("; passthru {} # {}", rule.domain, rule.comment.as_deref().unwrap_or(""));
    }
